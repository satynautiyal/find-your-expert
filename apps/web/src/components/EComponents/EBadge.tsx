...
export const badgeVariants = cva(
  'inline-flex items-center font-medium transition-colors select-none',
  {
    variants: {
      variant: {
        verified:
          'gap-1 font-bold text-[#F15A24] text-xs',
        verifiedPill:
          'gap-1.5 px-3 py-1 rounded-full bg-[#FFF5F0] border border-orange-200 text-[11px] font-bold text-[#F15A24] shadow-2xs',
        service:
        ...
        rank:
          'w-6 h-6 rounded bg-[#F15A24] text-white font-extrabold text-xs items-center justify-center shadow-xs',
        photoCount:
        ...
        popularChip:
          'px-2.5 py-1 rounded-md bg-gray-100/80 hover:bg-orange-50 hover:text-[#F15A24] text-gray-600 font-medium text-[11px] cursor-pointer',
      },
    },
    defaultVariants: {
      variant: 'service',
    ...
    return (
      ...
      >
        {variant === 'verified' && !icon && (
          <CheckCircle2 className="w-3.5 h-3.5 fill-[#F15A24] text-white shrink-0" />
        )}
